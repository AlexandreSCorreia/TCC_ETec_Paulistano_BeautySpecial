...
namespace CamadaLogicaDeNegocios_BLL
{
    public class ClassAgendamento_BLL : IClassAgendamento_BLL
    {
        ...
        public ClassAgendamento_Collection_DTO ConsultaPorNome(string nome)
        {
            try
            {
                ...
                foreach (DataRow linha in dataTableAgendamento.Rows)
                {
                    ...
                    //Agendamento
                    if (linha["ID_Agendamento"] != DBNull.Value) { classAgendamento_DTO.ID_Agendamento = Convert.ToInt32(linha["ID_Agendamento"]); }
        ...
        //Metodo Excluir
        public string ExcluirAgendamento(ClassAgendamento_DTO classAgendamento_dto)
        {
            try
            {
                //Limpando Parametros
                classacessobancodados.LimparParametros();
                //Passando parametros para procedure
                classacessobancodados.AdicionarParametos("@ID_Agendamneto", classAgendamento_dto.ID_Agendamento);
                //capturando ID ou uma string de erro que volta da procedure
                string IdAgend = classacessobancodados.Executa_ComandoSQL(CommandType.StoredProcedure, "uspDeletarAgendamento").ToString();
                // retornando id para o methodo
                return IdAgend;
            }
            catch (Exception ex)
